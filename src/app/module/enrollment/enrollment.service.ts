...
const enrollCourseInDB = async (userId: string, payload: ICreateEnrollmentPayload) => {
  ...
  for (const prereq of section.course.prerequisites) {
    const passedResult = await prisma.courseResult.findFirst({
      where: {
  ...
  // 7. Atomic Enrollment & Capacity update with extended transaction options
  const result = await prisma.$transaction(
    async (tx) => {
      // Re-verify capacity & status inside transaction for safety
      const currentSection = await tx.section.findUnique({
        where: { id: sectionId },
        select: { enrolledCount: true, capacity: true, status: true },
      });

      if (
        currentSection?.status !== "OPEN" ||
        currentSection.enrolledCount >= currentSection.capacity
      ) {
        throw new AppError(400, "Section is full or no longer open");
      }
